...
public class AStar : MonoBehaviour
{
    ...
    public static List<Vector2> FindPath(Vector2 start, Vector2 target, Vector2 gridCellSize, System.Func<Vector2, bool> isObstacle)
    {
        ...
        Vector2Int startGridPos = Gridtiles.WorldtoGrid(start);
        Vector2Int targetGridPos = Gridtiles.WorldtoGrid(target);
        ...
        while (openList.Count > 0)
        {
            ...
            {
                if (closedSet.Contains(adjacentPos) || isObstacle(Gridtiles.GridtoWorld(adjacentPos)))
                {
                    continue;
                }
    ...
    private static List<Vector2> GeneratePath(Node endNode)
    {
        ...
        {
            path.Add(Gridtiles.GridtoWorld(currentNode.position));
